...
|------|-------------------|
| `mock_gen.go` | A `MockService` struct implementing `Service`. Each method has a configurable behavior (`On("Send").Return(nil)` style) for use in tests. The struct embeds `contractkit.Recorder` so tests can assert call counts and captured arguments. |

`mock_gen.go` is regenerated on every `forge generate`. **Never
hand-edit it.** If the contract changes, edit `contract.go` and re-run.

Each generated file carries the canonical forge ownership header — a
`// Code generated by forge. DO NOT EDIT.` line, a `// Source:` pointer
at `contract.go`, and a `// To customize:` pointer back at the contract.
`forge lint --scaffolds` enforces both halves: hand-editing a `_gen.go`
...
`metrics_gen.go` — per-method wrappers around every contract interface.
Those were removed in 1.7+ in favour of:

- **Connect interceptors at the handler boundary** — covers the
  request-scoped case (the vast majority of observability needs)
  without per-package codegen. See `forge/pkg/observe`:
  `LoggingInterceptor`, `TracingInterceptor`, `MetricsInterceptor`,
  `RecoveryInterceptor`, `RequestIDInterceptor`. The canonical
  chain is `observe.DefaultMiddlewares(deps)`. The scaffolded
  `cmd/server.go` wires it for you.

- **Opt-in per-method helpers** — for the rare case where one
  Service calls another and you want a child span / log line /
  metric. Use `observe.LogCall` / `observe.TraceCall` /
  `observe.NewCallMetrics` at the explicit call site:
...
This is "observability is a property of the call site that wants it,
not a property of every interface", which is much more honest about
what the per-method wrappers were really doing. Existing projects on
the old shape are migrated by `forge generate` automatically — see the
`v0.x-to-observe-libs` migration skill.
...
func TestCreateUser_SendsWelcomeEmail(t *testing.T) {
    ...
    mockEmail.On("Send", mock.Anything, "alice@example.com", "Welcome", mock.Anything).
        Return(nil).
        Once()
...
- Never hand-edit `*_gen.go` files (`mock_gen.go`, `middleware_gen.go`, `tracing_gen.go`, `metrics_gen.go`). Edit `contract.go` and re-run `forge generate`.
