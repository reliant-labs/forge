...
For `--kind cli`, `forge generate` does NOT regenerate Connect-RPC stubs, frontend hooks, or `pkg/app/bootstrap.go` (none exist). It DOES still produce `mock_gen.go`, `middleware_gen.go`, `tracing_gen.go`, `metrics_gen.go` for any `internal/<name>/contract.go` it finds.
