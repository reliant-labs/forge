...
it. Use the library's helpers (`tdd.TableRPC`, `tdd.TableContract`,
`tdd.E2EClient`, `tdd.NewMock`, `tdd.AssertConnectError`,
`tdd.WithTimeout`, `tdd.SetupMockDB`) as the default vocabulary for new
test files. See `testing/patterns` for copy-paste-ready templates.
