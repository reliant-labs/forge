...
1. Comment-block the frontend deploy options into staging/prod `main.k` (cheap, high value)
2. Warn when a declared frontend has no deploy target for the env being deployed
3. Generalise `FirebaseHosting` → `StaticHosting` with a `provider` field, Firebase as provider #1
4. Auto-select static vs standalone by detecting dynamic routes
5. Revisit `SimpleBackend` once 1–4 land — several of its motivating cases may already be covered
