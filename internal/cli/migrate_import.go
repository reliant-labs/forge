...
// newMigrateImportCmd creates the `forge migrate import` subcommand. The
// only supported source today is goose; the --from flag exists so we can
// add `--from dbmate` / `--from sql-migrate` later without a flag-shape
// change. Renumbering picks up after the highest existing index in the
// target migrations dir, which keeps pack-installed migrations
// (00001-0000N) intact and slots the imported migrations behind them.
...
// runMigrateImport is the entrypoint for `forge migrate import`. It
// validates flags, scans the source directory, converts each file, and
// writes the results to the destination directory.
