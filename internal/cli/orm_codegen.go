...
func generateSharedHeader(g *protogen.GeneratedFile, file *protogen.File, _ bool) {
	g.P("// Code generated by forge. DO NOT EDIT.")
	g.P("// forge-owned: regenerated every run — do not edit (forge disown to take ownership)")
	g.P()
...
func generateEntityHeader(g *protogen.GeneratedFile, file *protogen.File, hasTimestamp bool, softDelete bool, hasTenant bool, hasWrapper bool) {
	g.P("// Code generated by forge. DO NOT EDIT.")
	g.P("// forge-owned: regenerated every run — do not edit (forge disown to take ownership)")
	g.P()
	g.P("package ", file.GoPackageName)
	...
	if hasWrapper {
		g.P(`	"google.golang.org/protobuf/types/known/wrapperspb"`)
	}
	g.P(")")
	g.P()
}

func generateEntityCode(g *protogen.GeneratedFile, ent entityInfo, _ bool) {
...
func generateValues(g *protogen.GeneratedFile, ent entityInfo, msgName string) {
	...
	for _, f := range ent.fields {
		...
		case isWrapperField(f.field):
			// Wrapper fields (google.protobuf.*Value): the proto-generated
			// entity field is *wrapperspb.XValue, but the DB column holds
			// the unwrapped scalar (nullable). Pass the unwrapped value
			// (or nil) to the driver — passing the wrapper struct directly
			// has no sql.driver.Valuer and the driver rejects it.
			g.P("		func() any { if ", accessor, " != nil { return ", accessor, ".Value }; return nil }(),")
		default:
			g.P("		", accessor, ",")
		}
	}
...
func generateScan(g *protogen.GeneratedFile, ent entityInfo, msgName string) {
	...
	for _, f := range ent.fields {
		...
		case isWrapperField(f.field):
			// Wrapper fields scan the unwrapped scalar as a pointer so a
			// NULL column maps to a nil entity field.
			_, scalar, _ := wrapperGoType(f.field)
			g.P("		", varName, " *", scalar)
		default:
	...
	for _, f := range ent.fields {
		...
		case isWrapperField(f.field):
			// Rewrap the scalar back into the entity's *wrapperspb.XValue
			// field. The entity field type comes from protoc-gen-go and
			// can't be assigned a bare *scalar — without the &XValue{Value:*v}
			// bridge `go build` fails with a type mismatch.
			wrapper, _, _ := wrapperGoType(f.field)
			g.P("	if ", varName, " != nil {")
			g.P("		m.", f.field.GoName, " = &", wrapper, "{Value: *", varName, "}")
			g.P("	}")
		case !f.notNull && !f.isPK && isNullableType(f.field):
			g.P("	if ", varName, " != nil {")
...
func generateCreate(g *protogen.GeneratedFile, ent entityInfo, msgName string, _ []string) {
	g.P("// Create", msgName, " inserts a new ", msgName, " row into the database.")
	if ent.tenantField != nil {
		g.P("func Create", msgName, "(ctx context.Context, db orm.Context, msg *", msgName, ", tenantID string) error {")
	} else {
	...
	g.P()
	if ent.tenantField != nil {
		g.P("	// Enforce tenant isolation: set tenant field from caller.")
		g.P("	msg.", ent.tenantField.field.GoName, " = tenantID")
		g.P()
	}
...
func generateGetByID(g *protogen.GeneratedFile, ent entityInfo, msgName string, pkCol string) {
	...
	if ent.tenantField != nil {
		g.P("func Get", msgName, "ByID(ctx context.Context, db orm.Context, id ", pkGoType, ", tenantID string) (*", msgName, ", error) {")
	} else {
	...
	g.P(`	defer span.End()`)
	g.P()
	if ent.softDelete || ent.tenantField != nil {
		// Use List with filters for soft-delete and/or tenant scoping.
		if ent.tenantField != nil {
			g.P(`	results, err := List`, msgName, `(ctx, db, tenantID, orm.WhereEq("`, pkCol, `", id), orm.WithLimit(1))`)
		} else {
...
func generateList(g *protogen.GeneratedFile, ent entityInfo, msgName string) {
	...
	if ent.tenantField != nil {
		g.P("func List", msgName, "(ctx context.Context, db orm.Context, tenantID string, opts ...orm.QueryOption) ([]*", msgName, ", error) {")
	} else {
	...
	if ent.tenantField != nil {
		g.P("	// Prepend tenant isolation filter.")
		g.P(`	opts = append([]orm.QueryOption{orm.WhereEq("`, ent.tenantField.columnName, `", tenantID)}, opts...)`)
		g.P()
	}
	...
	if ent.tenantField != nil {
		g.P("func Count", msgName, "(ctx context.Context, db orm.Context, tenantID string, opts ...orm.QueryOption) (int64, error) {")
	} else {
...
func generateListAll(g *protogen.GeneratedFile, ent entityInfo, msgName string) {
	...
	if ent.tenantField != nil {
		g.P("func ListAll", msgName, "(ctx context.Context, db orm.Context, tenantID string, opts ...orm.QueryOption) ([]*", msgName, ", error) {")
	} else {
	...
	g.P()
	if ent.tenantField != nil {
		g.P("\t// Tenant isolation still applies even for ListAll (bypasses soft-delete only).")
		g.P(`	opts = append([]orm.QueryOption{orm.WhereEq("`, ent.tenantField.columnName, `", tenantID)}, opts...)`)
		g.P()
	}
	g.P("\tresults, err := orm.List[", msgName, "](ctx, db, opts...)")
...
func generateUpdate(g *protogen.GeneratedFile, ent entityInfo, msgName string, _ []string, pkCol string) {
	g.P("// Update", msgName, " updates an existing ", msgName, " by its primary key.")
	if ent.tenantField != nil {
		g.P("func Update", msgName, "(ctx context.Context, db orm.Context, msg *", msgName, ", tenantID string) error {")
	} else {
	...
	if ent.softDelete || ent.tenantField != nil {
		// Use manual UPDATE to include soft-delete and/or tenant WHERE clauses.
		...
		if ent.softDelete && ent.tenantField != nil {
			...
			g.P(`	query := fmt.Sprintf("UPDATE %s SET %s WHERE %s = %s AND %s IS NULL AND %s = %s",`)
			...
			g.P(`		dialect.QuoteIdentifier("`, ent.tenantField.columnName, `"),`)
			g.P(`		dialect.Placeholder(`, tenantPlaceholderIdx, `),`)
			g.P("	)")
			g.P()
			g.P("	args := []any{")
			...
			// Tenant only, no soft-delete.
			tenantPlaceholderIdx := pkPlaceholderIdx + 1
			g.P(`	query := fmt.Sprintf("UPDATE %s SET %s WHERE %s = %s AND %s = %s",`)
			g.P(`		dialect.QuoteIdentifier("`, ent.tableName, `"),`)
			g.P(`		strings.Join(setParts, ", "),`)
			g.P(`		dialect.QuoteIdentifier("`, pkCol, `"),`)
			g.P(`		dialect.Placeholder(`, pkPlaceholderIdx, `),`)
...
func generateDelete(g *protogen.GeneratedFile, ent entityInfo, msgName string, pkCol string) {
	...
	if ent.tenantField != nil {
		g.P("func Delete", msgName, "(ctx context.Context, db orm.Context, id ", pkGoType, ", tenantID string) error {")
	} else {
	...
	if ent.softDelete && ent.tenantField != nil {
		g.P("	// Soft delete with tenant isolation.")
		g.P("	dialect := db.Dialect()")
		g.P(`	query := fmt.Sprintf("UPDATE %s SET %s = CURRENT_TIMESTAMP WHERE %s = %s AND %s IS NULL AND %s = %s",`)
		g.P(`		dialect.QuoteIdentifier("`, ent.tableName, `"),`)
		g.P(`		dialect.QuoteIdentifier("deleted_at"),`)
		g.P(`		dialect.QuoteIdentifier("`, pkCol, `"),`)
		...
		g.P(`		dialect.QuoteIdentifier("deleted_at"),`)
		g.P(`		dialect.QuoteIdentifier("`, ent.tenantField.columnName, `"),`)
		g.P(`		dialect.Placeholder(1),`)
		g.P("	)")
		g.P("	_, err := db.Exec(ctx, query, id, tenantID)")
	} else if ent.softDelete {
	...
	} else if ent.tenantField != nil {
		g.P("	// Hard delete with tenant isolation.")
		g.P("	dialect := db.Dialect()")
		g.P(`	query := fmt.Sprintf("DELETE FROM %s WHERE %s = %s AND %s = %s",`)
		g.P(`		dialect.QuoteIdentifier("`, ent.tableName, `"),`)
		g.P(`		dialect.QuoteIdentifier("`, pkCol, `"),`)
		g.P(`		dialect.Placeholder(0),`)
		g.P(`		dialect.QuoteIdentifier("`, ent.tenantField.columnName, `"),`)
