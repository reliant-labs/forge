...
// TestE2EScaffoldServerSetColumnTakesItsSchemaDefault pins the create path
// for a column the client cannot set.
//
// A `// forge:server-set` field is absent from the born create request, so
...
func TestE2EScaffoldServerSetColumnTakesItsSchemaDefault(t *testing.T) {
	...
	proto += "\nenum Tone {\n" +
		...
		"message Gadget {\n" +
		"  string name = 1;\n" +
		"  Tone tone = 2; // forge:server-set\n" +
		"  int64 priority = 3; // forge:server-set\n" +
		"  bool active = 4;\n" +
		"}\n"
	...
	// server-set field is a state the domain actually has. The two halves
	// compose — the column DEFAULT is what the create op pastes.
	if !strings.Contains(createOp, `e.Tone = "TONE_WARM"`) {
		t.Errorf("the create op must write the enum column's DEFAULT for a server-set field:\n%s", createOp)
	}
	if strings.Contains(createOp, "UNSPECIFIED") {
		t.Errorf("a server-set enum must not be born at the proto zero:\n%s", createOp)
	}

	// Evolution the charter tells authors to write by hand: a non-zero
	// default plus a range CHECK on the server-set column, and a
	...
	if !strings.Contains(createOp, "e.Priority = 5") {
		t.Errorf("the create op must write the hand-written DEFAULT 5 for a server-set column:\n%s", createOp)
	}
	...
	if strings.Contains(readFileE2E(t, filepath.Join(projectDir, "internal", "db", "gadget_orm.go")), "nullzero") {
