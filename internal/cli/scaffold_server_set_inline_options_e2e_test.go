...
// TestE2EScaffoldServerSetSurvivesInlineOptions pins the `// forge:server-set`
...
// the author believed the field was server-authoritative.
//
...
func TestE2EScaffoldServerSetSurvivesInlineOptions(t *testing.T) {
	...
	proto += "\nenum OrderStatus {\n" +
		...
		"  int64 unit_price_cents = 2 [(buf.validate.field).int64.gte = 0]; // forge:server-set\n" +
		"  OrderStatus status = 3; // forge:server-set\n" +
	...
	for _, serverSet := range []string{"unit_price_cents", "status", "term_months"} {
		if strings.Contains(createReq, serverSet) {
			t.Errorf("CreateOrderRequest kept the `// forge:server-set` field %q — the marker was dropped:\n%s",
				serverSet, createReq)
		}
	}
	...
	// The entity message keeps every field — server-set trims the WRITE
	// side only; the column and the read surface are untouched.
	entityMsg := sliceBetweenCLI(genProto, "message Order {", "}")
	for _, serverSet := range []string{"unit_price_cents", "status", "term_months"} {
		if !strings.Contains(entityMsg, serverSet) {
			t.Errorf("server-set field %q must stay on the entity message:\n%s", serverSet, entityMsg)
		}
	}
...
// TestE2EScaffoldServerSetMarkerWithNoFieldFailsLoudly pins the other half of
// the contract: a marker forge CANNOT attach to a field is refused by name,
// never dropped. A silently-ignored marker ships a server-authoritative field
// to clients while its author believes it is protected — so birth writes
// nothing and the command exits non-zero.
func TestE2EScaffoldServerSetMarkerWithNoFieldFailsLoudly(t *testing.T) {
	t.Parallel()
	forgeBin := buildforgeBinary(t)
	dir := t.TempDir()
	...
	if err == nil {
		t.Fatalf("`forge scaffold` exited 0 with a `// forge:server-set` marker that reached no field:\n%s", out)
	}
	text := string(out)
	for _, want := range []string{"forge:server-set", "orders.proto:", "attaches to no field"} {
