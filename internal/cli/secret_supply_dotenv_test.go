...
// A dotenv secret_provider supplies the Secrets it projects, even though it
...
//
// Regression: without the dotenv branch in secretSupplyForPreflight, the
...
func TestSecretSupplyForPreflight_DotenvProviderSuppliesDeclaredRefs(t *testing.T) {
	...
	for _, want := range []string{"github-oauth", "creds"} {
		if !got[want] {
			t.Errorf("dotenv supply missing Secret %q; got %v", want, got)
		}
	}
	if got["stripe"] {
...
func TestSecretSupplyForPreflight_DotenvDedupesBySecretName(t *testing.T) {
  ...
  "secret_provider": {"type": "dotenv", "path": ".env.dev"}
}`
