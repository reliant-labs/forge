...
// scaffolding path: no dir on disk → synthesized compact identity,
...
func TestResolveComponentDir_FallbackSynthesizesForNewComponents(t *testing.T) {
	...
	if res.ImportLeaf != "engineshadow" || res.PackageName != "engineshadow" {
		t.Errorf("fallback = %q/%q, want compact engineshadow/engineshadow", res.ImportLeaf, res.PackageName)
	}
	...
	if err != nil || res.FromDisk || res.PackageName != "calibratorrefit" {
		t.Errorf("empty projectDir: res=%+v err=%v, want synthesized calibratorrefit", res, err)
	}
}
