...
func TestGenerateServiceStub_ZeroMethods_NoUnusedImports(t *testing.T) {
	...
	if !strings.Contains(content, `Logger *slog.Logger`) {
		t.Error("generated stub should contain Logger field in Deps")
	}
...
`
	...
	// Should NOT create handlers_new.go
	if _, err := os.Stat(filepath.Join(targetDir, "handlers_new.go")); !os.IsNotExist(err) {
		t.Error("handlers_new.go should not be created when all methods exist")
	}
}
