...
//
//   - pre-push runs the project's OWN gates, `forge lint` + `forge audit`.
...
// baseline gate. `forge new` sets it at init time; a fresh clone self-heals
...
func (g *ProjectGenerator) generateGitHooks() error {
	...
	// pre-commit stays cheap — no `forge lint`/`forge audit` here (they run
...
# Kept fast: the heavy gates (forge lint + forge audit) run on pre-push,
