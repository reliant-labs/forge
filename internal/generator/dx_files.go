...
// untracked .git/hooks/), these are versioned with the repo and run the
// project's OWN gates — `forge lint` and `forge audit` — on every commit.
//
...
// .git/config makes the hook fire correctly in every linked worktree
// (reliant/forge lean heavily on worktrees). `forge new` sets it at init
// time; a fresh clone self-heals on the next forge command (see
// ensureGitHooksActivated in internal/cli). No `pre-commit install`, no
// Python dependency for the baseline gate.
//
// core.hooksPath overrides .git/hooks entirely, so the pre-commit
// framework's own hooks (gitleaks, gofmt, prettier, buf, commitlint)
// would otherwise stop firing locally. We chain to them explicitly — a
// no-op when `pre-commit` isn't installed — so nothing regresses for
// contributors who use the framework, while CI enforces it regardless
...
# core.hooksPath overrides .git/hooks, so the pre-commit framework's hooks
# (gitleaks, gofmt, prettier, buf) won't fire on their own — invoke them
# here when installed. No-op when pre-commit isn't present; CI runs them
# regardless via .github/workflows/pre-commit.yml.
