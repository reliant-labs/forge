...
func (g *ProjectGenerator) Generate() error {
	...
	if g.isService() {
		dirs = append(dirs, "internal/handlers", "internal/handlers/mocks", "internal/cli", "pkg/app", "pkg/middleware")
	}
	if g.hasCmd() {
	...
	case g.isService():
		...
		// Thin cmd/main.go (gh / cobra-cli idiom) — all command logic lives
		// in the internal/cli package. binary=shared only changes the
		// doc text; both modes are a thin cli.Execute().
		mainTmpl := "cmd-root.go.tmpl"
		if g.isBinaryShared() {
			mainTmpl = "cmd-shared-main.go.tmpl"
		}
		files = append(files,
			struct{ template, dest string }{mainTmpl, "cmd/main.go"},
			// internal/cli command tree (one file per command).
			struct{ template, dest string }{"cli-root.go.tmpl", "internal/cli/root.go"},
			struct{ template, dest string }{"cli-version.go.tmpl", "internal/cli/version.go"},
		)
		// internal/cli/commands.go — the user-owned cobra extension point
		...
		files = append(files, struct{ template, dest string }{"cli-commands.go.tmpl", "internal/cli/commands.go"})
	case g.isCLI():
	...
	// internal/cli/{serve,server,db}.go import pkg/config and pkg/app
	...
	if g.isService() && g.Features.CodegenEnabled() {
		files = append(files,
			struct{ template, dest string }{"cli-serve.go.tmpl", "internal/cli/serve.go"},
			struct{ template, dest string }{"cli-server.go.tmpl", "internal/cli/server.go"},
		)
		// internal/cli/db.go (migrate CLI) depends on both pkg/config and
		// golang-migrate; skip when migrations are also disabled.
		if g.Features.MigrationsEnabled() {
			files = append(files, struct{ template, dest string }{"cli-db.go.tmpl", "internal/cli/db.go"})
		}
	}
