...
|-------|--------|-----------|
| **Pure forge-space** | `// Code generated by forge. DO NOT EDIT.` + `// forge-owned: regenerated every run — do not edit (forge disown to take ownership)` header pair (usually a `_gen` filename, but also `internal/db/*_orm.go` and `internal/db/types.go`) + `// Source: …` pointer | Regenerated on every `forge generate`. User edits are blown away (or take the one-way door: `forge disown <path>`). Listed in `.gitignore` (not committed). |
