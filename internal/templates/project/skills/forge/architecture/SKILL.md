...
> **This is the canonical target shape. Forge is migrating to it.** Older docs and existing trees may still show top-level `handlers/`, `pkg/app/{bootstrap,setup,wire_gen}.go`, a string-keyed service registry, and `serverkit.Run(..., names)`. That is the deprecated layout — the shape below is authoritative, and the skills are being rewritten to regenerate it.
...
internal/                 # DEFAULT HOME for everything not imported outside the module
  <svc>/                  #   a service: contract.go + impl + handlers_gen.go co-located in ONE dir
  <worker>/               #   workers live under internal/, NOT top-level workers/
  <operator>/             #   operators live under internal/, NOT top-level operators/
...
internal/<svc>/               # service: contract + impl + generated handlers, co-located
...
Why not top-level `handlers/`, `workers/`, `operators/`, or `pkg/app`: a top-level directory falsely advertises public API in an about-to-be-open-sourced repo. These are imported by nobody external, so they belong under `internal/`. The old `handlers/<svc>/ → internal/<svc>/` two-tier split (6–7 files across 2 dirs, ~25% pure shim forwarders) is forge-scaffolded indirection — collapse the thin-forwarder tier and co-locate generated + owned files in one `internal/<svc>/`. (Forge's own shipped libraries under `forge/pkg/*` are a different module — genuine library code, and they stay.)

## Generated vs Hand-Written

| Forge generates (safe to regenerate) | You own (Forge never touches) |
|--------------------------------------|-------------------------------|
| `gen/` — Go stubs, TS clients, mocks | `internal/<svc>/<svc>.go` + `contract.go` — business logic |
| `internal/<svc>/handlers_gen.go` — Connect handlers | `internal/app/build.go` — the composition root |
| `internal/middleware/auth_gen.go`, `tenant_gen.go` — auth/tenant codegen | `internal/db/` — mappers, custom queries, schema |
| `internal/<svc>/authorizer_gen.go` — generated RBAC | `internal/<svc>/authorizer.go` — custom auth |
| `internal/db/<entity>_orm.go` — entity struct + ORM (projected from the applied schema) | `internal/<svc>/handlers_crud.go` — thin CRUD delegations (scaffolded once, appended-to for new RPCs) |
...
db/migrations/*.up.sql (applied to a real ephemeral postgres shadow DB, introspected)
  ...
  → forge generate → internal/db/<entity>_orm.go (entity struct + ORM)
                     internal/<svc>/handlers_crud_ops_gen.go (per-RPC ops + ToProto/FromProto conversions)
                     frontend pages, nav, mocks

internal/<svc>/contract.go (Go interfaces)
  → forge generate → mock_gen.go

forge_descriptor.json + internal/<svc>/
  → forge generate → internal/<svc>/handlers_gen.go, authorizer_gen.go
...
| **Internal packages** | Go interfaces (`internal/<svc>/contract.go`) | Compile-time + contract linter |
...
- `internal/<svc>/*_gen.go` — Generated handlers, CRUD op constructors, and authorizers (your CRUD method bodies live in the user-owned `handlers_crud.go`)
...
3. **Implement** business logic in `internal/<svc>/<svc>.go` behind the `contract.go` interface
...
| `forge.yaml` `path:` field (and on-disk directory) | lowercase snake_case (`-` → `_`, case boundaries split) | `internal/admin_server`, `internal/git_credential` |
| Go package directory under `internal/` | lowercase snake_case | `internal/admin_server/`, `internal/billing_flow/` |
...
**Same identifier, three forms in one sentence.** When discussing `admin-server`'s `AdminServer.GetUser` RPC handler in `internal/adminserver/handlers_gen.go` — that's the kebab-case service display name, the PascalCase RPC method, and the canonicalized directory path. All three are intentional and all three are correct. The service / worker / operator canonicalization rule (`strings.ToLower` then strip `-` and `_`) lives in `generator.ServicePackageName`; the `workers` skill Naming section is the source of truth for the on-disk consequences and the migration gotcha.
...
- One service per proto package. One `internal/<svc>/` directory per service, co-locating contract + impl + generated handlers.
