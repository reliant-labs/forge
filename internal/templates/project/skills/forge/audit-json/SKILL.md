...
| `unscoped_auth` | `authenticated_rpcs`, `scoped_rpcs`, `unscoped_rpcs[]` (`{service, method, file, delegating, entity, table}`), `owner_scoped_unscoped_rpcs[]` (the GATING subset — same shape, always carrying `entity`/`table`), `owner_scoped_tables[]`, `owner_marker`, `acknowledged_rpcs[]` (`{service, method, file, reason}`), `auth_seam`, `acknowledge_marker`, `hint` — RPCs whose proto declares `auth_required: true` but whose handler body never resolves the caller, so every signed-in user reaches every row. The authenticated set is read from `gen/forge_descriptor.json` (the same `auth_required` projection that drives `pkg/middleware/procedures_gen.go`); the reads-the-caller set is the handler AST, matched against the scaffolded auth seam; the owned-table set is read from the migrations' `COMMENT ON COLUMN ... IS 'forge:owner'` declarations. **Status is `warn` when `unscoped_rpcs` is non-empty, and `error` when `owner_scoped_unscoped_rpcs` is** — a project that declares no owner column stays advisory (a fresh scaffold is unscoped by construction), while one that declared where its rows' owner lives fails the gate for the RPCs crossing that boundary. The RPC→table join is the CRUD-name derivation, so a custom (non-CRUD) RPC cannot arm the gate however many owned tables it touches; it still reports as a warning. An intentionally global RPC is declared IN CODE above the handler with `// forge:auth-unscoped-ok: <reason>` — this suppresses the error as well as the warning; the reason is required and is echoed back in `acknowledged_rpcs` for review. If the project declares authenticated RPCs and NO handler matched any of them, the category reports `warn` saying it inspected nothing rather than a vacuous `ok`. |
