...
Every service domain in a forge project lives in ONE `internal/<svc>/` directory — owned and generated files co-located, no two-tier `handlers/<svc>/` + `internal/<svc>/` split:
...
internal/things/
  ...
  mock_gen.go        # generated by `forge generate` — your test seam, free
```

`forge generate` reads `contract.go` and emits `handlers_gen.go` + `mock_gen.go` alongside your owned files.
