...
Every service domain in a forge project has the same shape:
...
`forge generate` reads `contract.go` and emits `mock_gen.go` — your test seam, free.
...
func Setup(app *App) error {
    app.Things, _ = things.New(things.Deps{
        DB:    app.DB,
        Now:   time.Now,
        NewID: ulid.Make,
    })
...
The service body and `setup.go` carry no observability code — `observe.DefaultMiddlewares(deps)` wraps it at the handler boundary.
...
- **Wire concrete dependencies in `pkg/app/setup.go`.** `bootstrap.go` is generated; don't edit it. Observability is applied at the handler boundary by `forge/pkg/observe` interceptors.
