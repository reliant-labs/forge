...
const variantConfig: Record<ToastVariant, { bg: string; icon: string; border: string }> = {
  success: { bg: "bg-success-surface", icon: "text-success", border: "border-success-border" },
  error: { bg: "bg-danger-surface", icon: "text-danger", border: "border-danger-border" },
  warning: { bg: "bg-warning-surface", icon: "text-warning", border: "border-warning-border" },
  info: { bg: "bg-accent-surface", icon: "text-accent", border: "border-accent-border" },
};
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Icon-only control: aria-label is the only name it can have. */}
      <button type="button" aria-label="Dismiss notification" onClick={() => onDismiss(toast.id)} className="flex-shrink-0 text-ink-subtle hover:text-ink-muted">
